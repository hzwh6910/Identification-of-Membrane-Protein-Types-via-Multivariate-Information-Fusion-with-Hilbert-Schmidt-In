...
function H=HOG(Im)
nwin_x=5;%set here the number of HOG windows per bound box
nwin_y=5;
B=10;%set here the number of histogram bins
...
hx = [-1,0,1];
hy = -hx';
grad_xr = imfilter(double(Im),hx); % 滤波
grad_yu = imfilter(double(Im),hy);
angles=atan2(grad_yu,grad_xr); % 返回点与x轴正向的夹角
magnit=((grad_yu.^2)+(grad_xr.^2)).^.5; % .5 是求开方
for n=0:nwin_y-1
    for m=0:nwin_x-1
        cont=cont+1;
