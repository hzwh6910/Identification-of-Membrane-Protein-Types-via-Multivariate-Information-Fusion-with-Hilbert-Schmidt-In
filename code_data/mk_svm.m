function [accuracy,predict_y,Scores,kernel_weights] = mk_svm(train_x,feature_id,train_y,test_x,test_y,c,gamma_list)
...
cg_str=['-t 4 -c ' num2str(c) ' -b 1 -q'];
model = svmtrain(train_y, K_train_com, cg_str);
K_test_com = [(1:num_test_samples)',K_test_com];
[predict_y, accuracy, Scores] = svmpredict(test_y,K_test_com,model,'-b 1');
